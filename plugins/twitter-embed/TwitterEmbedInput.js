...
export default class TwitterEmbedInput extends Component {
  ...
  fetchTweet = async () => {
    ...
    const res = await fetch(
      `https://www.sanity.work/api/get-tweet-data?statusUrl=${statusUrl}`
    )
    ...
    if (json?.data) {
      const patch = PatchEvent.from(
        set({
          ...
          _key:
            this.props.value?._key || Math.random().toString().replace('.', ''),
        })
      )
    ...
    return (
      ...
      >
        ...
        <DefaultFormField
          ...
          description={
            'Example: https://twitter.com/sanity_io/status/1354031530528694274'
          }
