...
export const TwitterShare = withDocument(
  React.forwardRef((props, ref) => {
    ...
    const handleClick = async (event) => {
      const contributionData = await client.fetch(`*[_id == $contributionId][0]`, {
        contributionId: document.contribution._ref,
      });

      const authorRefs = contributionData.authors.map((author) => author._ref);

      const authors = await client.fetch(`*[_id in $authors] {"handle": social.twitter, name}`, {
        authors: authorRefs,
      });
      const authorHandles = authors.map((author) =>
        author.handle ? `@${author.handle.split('/').pop()}` : author.name
      );

      const tweetString = `🎉 A new ${
        TYPES_TEXT[contributionData._type]
      } was published by ${authorHandles.join(', ')}: \n\n${
        contributionData.title
      } \n\nhttps://sanity.io/${TYPES[contributionData._type]}/${
        contributionData.slug.current
      }?utm_source=twitter&utm_medium=social&utm_campaign=exchange_bot`;
    ...
    return (
      <Stack space={2}>
        ...
        <Grid gap={2} columns={2}>
          ...
          >
            <Button fontSize={[1, 1, 2]} mode="ghost" padding={3} text="Copy Tweet" />
          </CopyToClipboard>
        </Grid>
      </Stack>
    );
